<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto">
        <x-card>
            <form action="{{ route('admin.kerusakan.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="kode_kerusakan" id="kode_kerusakan" value="{{ old('kode_kerusakan') }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_kerusakan" id="nama_kerusakan" value="{{ old('nama_kerusakan') }}" required
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3"
                              ...
                              placeholder="Penjelasan detail tentang kerusakan ini...">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="solusi" id="solusi" rows="4"
                              class="w-full rounded-xl border-border focus:ring-primary focus:border-primary px-4 py-2 @error('solusi') border-danger @enderror"
                              placeholder="Langkah-langkah perbaikan...">{{ old('solusi') }}</textarea>
                    @error('solusi')
                        <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                    @enderror
                </div>
