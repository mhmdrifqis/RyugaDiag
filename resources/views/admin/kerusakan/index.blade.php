<x-app-layout>
    ...
    <x-card>
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <form action="{{ route('admin.kerusakan.index') }}" method="GET" class="w-full md:w-1/3">
                <div class="relative">
            ...
            <div class="flex flex-col sm:flex-row w-full md:w-auto gap-2" x-data="{ showImportModal: false }">
                ...
                <a href="{{ route('admin.kerusakan.create') }}" class="w-full sm:w-auto">
                        ...
                        <div x-show="showImportModal" x-transition.scale.origin.bottom class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                            <div>
                                ...
                                <div class="mt-3 text-center sm:mt-5">
                                    <h3 class="text-lg font-bold text-primary" id="modal-title">Import Data Kerusakan</h3>
                                    <div class="mt-2 text-sm text-text-muted text-left">
                                        ...
                                        <ul class="list-disc pl-5 mt-2 font-mono text-xs text-primary space-y-1">
                                            ...
                                            <li>nama_kerusakan <span class="text-danger">*</span> (Wajib)</li>
                                            <li>deskripsi (Opsional)</li>
                                            <li>solusi (Opsional)</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            <form action="{{ route('admin.kerusakan.import') }}" method="POST" enctype="multipart/form-data" class="mt-5 sm:mt-6">
                                @csrf
                                <div class="mb-4">
            ...
            <table class="w-full text-left text-sm text-text-main">
                <thead class="bg-bg-light text-text-muted uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <th class="px-6 py-4 border-b border-border">Nama Kerusakan</th>
                        <th class="px-6 py-4 border-b border-border">Deskripsi</th>
                        <th class="px-6 py-4 border-b border-border">Solusi</th>
                ...
                <tbody class="divide-y divide-border bg-white">
                    @forelse ($kerusakans as $index => $kerusakan)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-center">{{ $kerusakans->firstItem() + $index }}</td>
                            <td class="px-6 py-4 font-semibold text-primary text-center">{{ $kerusakan->kode_kerusakan }}</td>
                            <td class="px-6 py-4">{{ $kerusakan->nama_kerusakan }}</td>
                            <td class="px-6 py-4 text-text-muted">{{ Str::limit($kerusakan->deskripsi, 50) ?: '-' }}</td>
                            <td class="px-6 py-4 text-text-muted">{{ Str::limit($kerusakan->solusi, 50) ?: '-' }}</td>
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <a href="{{ route('admin.kerusakan.edit', $kerusakan->id) }}" class="text-accent hover:text-accent-hover mr-3" title="Edit">
                                    <i class="fa-solid fa-pen-to-square text-lg"></i>
                                </a>
                                <form action="{{ route('admin.kerusakan.destroy', $kerusakan->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
